...
namespace MisFinanzas.Tests
{
    public abstract class MisFinanzasTestBase : AbpIntegratedTestBase<MisFinanzasTestModule>
    {
        ...
        protected void UsingDbContext(Action<MisFinanzasDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MisFinanzasDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MisFinanzasDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MisFinanzasDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MisFinanzasDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MisFinanzasDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MisFinanzasDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MisFinanzasDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MisFinanzasDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MisFinanzasDbContext, Task<T>> func)
        {
            T result;
